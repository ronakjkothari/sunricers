/* Intervention lab — lives on the Impact map tab (UI-Overhaul layout).
   ...
   All arithmetic lives in ../lib/levers.js; this file only draws. Without data/levers.json the lab falls back to a short empty state. */
...
export function mount(el, context) {
  ...
  root.innerHTML = `
    <section class="card panel" style="margin-top:0">
      <header class="labhead">
        ...
        <button class="btn sm" id="sc-showonmap">${icon("map", 14)} Jump to map</button>
  ...
  root.querySelector("#sc-showonmap").onclick = () => (ctx.scrollToMap ? ctx.scrollToMap() : ctx.showOnMap());
...
function changed() {
  ...
  drawAnswer();
  drawDetail();
  if (typeof ctx.onLabLeversChanged === "function") ctx.onLabLeversChanged();
  ctx.leversChanged();
}
...
export const mountLab = mount;
export const updateLab = update;
